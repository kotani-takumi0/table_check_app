...
- 退店から5分で空席表示に戻ります。
- 詳細パネルの「○番を移動」で、パネルを開いた卓を別の空席に付け替えます。「卓を追加（団体）」で空席を足すと、1組の客として状態・タイマー・お会計を共有します。どちらもボタンを押したあと移動先・追加先の空席をタップします（「やめる」か Esc で中止）。団体の卓はカードに「+12」のように他の卓番を出し、パネルの「×」で外せます。
- 縦向きの画面では、手描きの配置図と同じ向き（横向きの配置を時計回りに90°）で並べます。
- スマホ（幅か高さが600px以下）では、カードに卓番・短い状態名・タイマーだけを出します。空席のタップで案内し、使用中の卓はタップで詳細パネルを開いて、進める・戻す・お会計・時刻の修正を行います。
...
- 購読開始時から過去12時間以内のセッションのうち、`tables/{卓}.sessionId` がそのセッションを指し、かつセッションの `tableIds` にもその卓がある卓に表示します。同時案内では卓の参照を後から書いたセッションが表示されます。
